...
cp $PROG_DIR/workerConfig.template.yml $WORK_DIR/workerConfig.yml
sed -i "s|%TARGET_REPO%|$TARGET_REPO|g" $WORK_DIR/workerConfig.yml
sed -i "s|%REPO_ROOT%|$REPO_ROOT|g" $WORK_DIR/workerConfig.yml
sed -i "s|%BRANCH_NAME%|$BRANCH_NAME|g" $WORK_DIR/workerConfig.yml
sed -i "s|%API_TOKEN_SECRET%|$API_TOKEN_SECRET|g" $WORK_DIR/workerConfig.yml
sed -i "s|%KEY_LOCATION%|$KEY_LOCATION|g" $WORK_DIR/workerConfig.yml
sed -i "s|%KNOWN_HOSTS%|$KNOWN_HOSTS_FILE|g" $WORK_DIR/workerConfig.yml
sed -i "s|%TEST_EXECUTION_MANAGER_URL%|$TEST_EXECUTION_MANAGER_URL|g" $WORK_DIR/workerConfig.yml
sed -i "s|%WORKER_URL%|$WORKER_URL|g" $WORK_DIR/workerConfig.yml
export HOME=/opt/testeditor

export DISPLAY=:99.0

cd $WORK_DIR
exec $PROG_DIR/bin/test-editor-testexecution worker server $WORK_DIR/workerConfig.yml
